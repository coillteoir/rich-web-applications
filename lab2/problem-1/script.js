...
var generateContact = function (form) {
  var inputData = Array.from(form.childNodes).filter(function (elm) {
    return elm.nodeName == "INPUT";
  });
  var name = inputData.find(function (x) {
    return x.name == "fName";
  }).value;
  var number = inputData.find(function (x) {
    return x.name == "number";
  }).value;
  var email = inputData.find(function (x) {
    return x.name == "email";
  }).value;
  inputData.find(function (x) {
    return x.name == "fName";
  }).value = "";
  inputData.find(function (x) {
    return x.name == "number";
